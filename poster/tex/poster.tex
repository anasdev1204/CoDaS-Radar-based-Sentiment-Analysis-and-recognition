...
\begin{itemize}
\item \textbf{Method 1:} Classical machine learning on kinematic, behavioral, and spatial features extracted from radar signals.
\item \textbf{Method 2:} A transformer-based architecture trained end-to-end on raw radar and infrared sensor data.
\item \textbf{Method 3:} A neural network framework that jointly performs radar signal interpolation and sentiment classification.
\end{itemize}
...
\caption{Transformer-based pipeline: a spatial Point Cloud Encoder processes 3D points per frame, followed by a temporal encoder over the resulting sequence of latent vectors.}
\end{figure}
\vskip2ex
...
\bottomrule
\end{tabularx}
\caption{Performance metrics of supervised classification models.}
...
\bottomrule
\end{tabularx}
\caption{Grid search results for one-shot unsupervised classification. The split column indicates the number of train, validation, and test behavior classes.}
...
\begin{figure}
\centering
\includegraphics[width=0.99\columnwidth]{haider_workflow.jpeg}
