...
Positional Embeddings in infrared BERT & Yes/No \\
...
To compare the sampled models and optimize the hyperparameters, a simple supervised classification task is used for classifying complete recordings among different behavior classes. The dataset is split into train, validation and test sets using the user\_ids to avoid any personal style or patterns of a given user being leaked to the model during training. We place 4 random users in the test set and 2 users in the validation set with each user having performed 8 repetitions. Rest of the recordings are used for training (approx. 160 recordings per class). We set output projection size of the above model to the number of behavior classes, train it with cross entropy loss, while using an early stopping strategy with a patience of 5 epochs. The model is trained for a maximum of 100 epochs with a batch size of 32 and a learning rate of $10^-3$ with the AdamW optimizer. The model with the lowest validation loss is saved and evaluated on the test set using weighted classification F1 score. All experiments were executed on an NVIDIA GeForce RTX 4070 Laptop GPU with 8 GB of GDDR6 VRAM.

\subsubsection{Unsupervised Contrastive Learning}

Downstream applications of a model that classifies radar or infrared recordings among the classes of RF-Behavior seem limited. To match the generalized input capability of our model, we explore the possibility of having a generalized output capability as well. Typically, the classification head of a model can be removed to use it as a feature extractor for transfer learning or fine-tuning. But training for rigid labels may prime the model to ignore many other latent features it could have been able to extract so we train an embedding model instead of a classification model.

We set the model output size to 256 and perform a grid search over other hyperparameters. We train these models in a semi supervised fashion using the NT-Xent loss introduced in the SimCLR framework \cite{chen2020simple} to increase the cosine similarity between embedding vectors of different instances of the same class and decrease their similarity with all other instances of other classes in the batch. We split the dataset by completely moving two behaviors to validation and two behaviors to test sets. We pick the model with the lowest validation loss and then evaluate it's embeddings in a 1-shot fashion on the 2 unseen test classes. The NT-Xent loss for a positive pair $(i, j)$ and temperature $\tau$ is
\begin{equation}
    \ell_{i,j} = -\log \frac{\exp(\text{sim}(\mathbf{z}_i, \mathbf{z}_j)/\tau)}{\sum_{k=1}^{2N} \mathbb{1}_{[k \neq i]} \exp(\text{sim}(\mathbf{z}_i, \mathbf{z}_k)/\tau)} \,,
\end{equation}

One of the modalities will have a lower performance than the other and in order to enable the model for that modality to learn from all other data that we have available, we align the embedding space of all modalities and train both models jointly. Since the sensor data from different modalities was collected simultaneously and radar and infrared data are already synchronized, we can use a symmetric contrastive loss (like CLIP \cite{radford2021learning}) to align the two modalities. We map data from all sensor types onto embedding vectors of the same size and use cross entropy to increase the similarity between embeddings from different modalities but of the exact same recording instance and decrease the similarity with any other instance of the same class.

\subsection{Results}

Through our grid search, we discovered that using a classification token (CLS) pooling in the point cloud encoder (PCE) collapses the model's representation and all inputs are mapped to almost the same vector by the model. We also measured that by downsampling the time dimension with convolutional blocks in the infrared model, the model inference and convergence time is significantly reduced during training as scaled dot product attention is quadratically related to the sequence length. A heavier PCE also increases the inference time for radar models. We expect this to be caused by our iterative handling of jagged points dimension. For unsupervised models, we observed that mean pooling outperforms the end-of-sequence token (EOS) pooling and that a low training loss is also important and not just a low validation loss for high test performance. We interpret stopping early for a low validation loss at a high training loss as noisy and lacking convergence.

The results for the best supervised models are shown in Table \ref{tab:supervised_results} and the distribution of all our trained models is illustrated via a scatter plot in Figure \ref{fig:f1_scatter_plot}.
...
\bottomrule
\end{tabular}
\caption{Grid Search: Supervised Classification Results.}
...
\begin{figure}[!t]
    ...
    \caption{Scatter plot of F1-scores of supervised classification models from grid search.}
    \label{fig:f1_scatter_plot}
\end{figure}

For campaign 3, in addition to grid search, we also employed genetic search to crossover and mutate the hyperparameters of two out of top 5 models to create a new model in an attempt to maximize the classification F1 score but no new model was discovered within our compute budget that out performed the existing ones. We expect this could be due to subtle nature of the behaviors in this campaign and we might need more landmarks or sensors to capture the micro-movements associated with these behaviors. We also observed that the radar modality consistently underperformed compared to infrared modality across all campaigns, which could be attributed to the lower resolution, sparse sampling and higher noise levels in radar data.

The unsupervised models are evaluated for one-shot classification. All samples in the test set are embedded and one instance from each class is designated as the reference embedding and the rest are used as query embeddings. The cosine similarity between each query and all references is calculated and the class of the reference with the highest similarity is assigned to the query. This is repeated in a k-fold manner by choosing each sample as the reference once. The results for the best unsupervised models are shown in Table \ref{tab:unsupervised_results}. The unsupervised models show that they have to potential to generalize to unseen behaviors and achieve comparable classification performance with just one reference recording per class and also fewer model parameters.
...
\bottomrule
\end{tabular}
\caption{Grid Search: Unsupervised 1-Shot Classification Results. (\# Behaviors column follows the format: \#train-\#valid-\#test)}
...
For better generalization, the recordings in the training set can be augmented by 1) excluding random sensors from the 13 radars, 2) dropping random points from the heavier radar frames, 3) dropping random frames, 4) truncating the recordings to a random length, 5) adding Gaussian noise to the points, and 6) applying random geometric transformations such as 3D rotation, translation and scaling.

Our deep learning components are compatible with all raw modalities of RF-Behavior. A unified model architecture could be designed with a shared temporal encoder head and modality specific necks composed of a downsampler and/or a point cloud encoder. This single model could be trained on the entire dataset including all modalities and behavior classes simultaneously to learn a generalized embedding space. The embedding space could also be aligned with a language model embedding space to enable zero-shot classification of behaviors based on textual descriptions.
