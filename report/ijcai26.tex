...
\begin{abstract}
Sentiment analysis is crucial for human-computer interaction and is usually done on text, speech or visual data, it allows the computer to determine the human's emotion, improve human-computer communication and also helps in psychology. Most of the datasets for sentiment analysis and recognition are limited in scope and number of modalities while the RF-Behavior dataset consists of multiple modalities and human behaviors, we will focus on three different approaches of using radar to perform the analysis and recognition. The first approach consists of extracting various features from the radar sensor data and training classical Machine Learning (ML) models on it, the second approach trains a transformer on radar and infrared sensor data while the third approach trains a classification and interpolation model on the radar sensor data.
\end{abstract}
...
One of the biggest limitations of interpolation was the small number of samples, this can be somewhat alleviated by adding the other modalities in the dataset such as Infrared, this would allow the model to learn patterns of other modalities and use them to estimate the radar patterns. This can be further improved by interpolating for all modalities combined as some modalities may be active while others are inactive at a given timestamp, combining them all improves the possibility of even a single modality being active at a time.
The classification performance will also generally improve with additional features but it would be best to train the classifier separately on the entire sequence of a sample. Additionally, we can check the performance of the classifier on subsets of emotions instead.
