...
Extracting features from the radar data and using classical machine learning methods was able to achieve decent results that can still be improved upon once certain limitations are addressed. The most important limitation was the small number of samples, after preprocessing and spliting the frames into windows, the samples were reduced to 600 which is still a small number for training a model. This can be improved by adding more samples to the dataset, or by using data augmentation techniques such as adding noise to the features, or by using generative models to create synthetic samples. Another way to improve performance that was not explored in this work was to use other modalities in the dataset such as Infrared in addition to the radar, this would allow the model to learn patterns of other modalities and use them to estimate the radar patterns
