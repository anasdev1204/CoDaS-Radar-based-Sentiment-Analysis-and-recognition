...
A U-Net is a Convolutional Neural Network (CNN) whose input and output are the same shape achieved by continually downsampling via Max pooling and then upsampled using transpose convolution layers with skip connections resulting in a U shape, the classifier head is attached to the bottleneck located between the downsampling and upsampling layers and consists of an adaptive average pooling to reduce the size to 1 along with a simple MLP classifier. Instead of the predicted values being obtained separately by inputting target timestamps, the input sequence has dummy values at those positions or indices and later retrieved from the output at those same indices. The model is expected to fill in the blanks, so to speak.

A TCN is a CNN with causal padding where successive layers have greater dilation allowing each point of the output to see the entire most of the surrounding input sequence. The classifier head is a linear layer attached to the end of the output sequence. It has the same input as U-Net where the model learns to fill the dummy values at target timestamps.
...
U-Net and TCN were also trained in an ensemble with the baseline where the loss was calculated using model output and the difference between target values and interpolated values, the mean was not normalized to mean 0 for these experiments.
