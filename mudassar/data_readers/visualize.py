...
class MatPlot3D:
    ...
    def new_figure(
        ...
        # figure size
        if isinstance(figure_scale, tuple):
            figsize = figure_scale
        elif figure_scale and figure_scale > 0:
            sizes: List = np.ptp(np.stack([x_limits, y_limits, z_limits]), axis=1).tolist()
    ...
    def initialize_Axes3D(
        ...
        if not ticks_scale:
            ticks_scale = _nearest_scale(
                np.ptp([x_limits, y_limits, z_limits], axis=1).min()
            )
        ...
        ax.set_box_aspect(
            np.ptp([x_limits, y_limits, z_limits], axis=1)[
    ...
    def animate(
        ...
        if not ticks_scale:
            ticks_scale = _nearest_scale(np.ptp(limits, axis=1).min())
        ...
        animation = FuncAnimation(
            ...
            frames=list(zip(frames, scatter_colors if scatter_colors is not None else [None] * len(frames))),
            interval=interval,
            blit=blit,
